...
def _get_tpex_price(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            price_raw_data = util.get_tpex_data(DataType.PRICE, date)
            price_df = util.clean_tpex_data(DataType.PRICE, price_raw_data)
            return price_df
        except:
...
def _get_tpex_fundamental(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            fundamental_raw_data = util.get_tpex_data(DataType.FUNDAMENTAL, date)
            fundamental_df = util.clean_tpex_data(DataType.FUNDAMENTAL, fundamental_raw_data)
...
def _get_tpex_margin_trading(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            margin_trading_raw_data = util.get_tpex_data(DataType.MARGIN_TRADING, date)
            margin_trading_df = util.clean_tpex_data(DataType.MARGIN_TRADING, margin_trading_raw_data)
...
def _get_tpex_institutional(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            institutional_raw_data = util.get_tpex_data(DataType.INSTITUTIONAL, date)
            institutional_df = util.clean_tpex_data(DataType.INSTITUTIONAL, institutional_raw_data)
