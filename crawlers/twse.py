...
def get_twse_final(date):
    ...
    try:
        ...
        df = pd.merge(
            df, twse_margin_trading_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        df = pd.merge(
            df, twse_institutional_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        df = pd.merge(
            df, twse_hold_percentage_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        # 沒有三大法人買賣超的補上0
        df[
            [
...
def _get_twse_price(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            r = requests.post(
                f"https://www.twse.com.tw/exchangeReport/MI_INDEX?response=csv&date={new_date}&type=ALL"
            )
            # 整理資料，變成表格
            df = pd.read_csv(
            ...
            # 合併漲跌數據
            df["漲跌"] = [
                str(x) + str(y) for x, y in zip(df["漲跌(+/-)"], df["漲跌價差"])
            ]
            # 僅保留所需欄位
            df = df.drop(
                [
            ...
            print(f"Attempt {_get_twse_price.__name__} failed.")
            time.sleep(3)
    return pd.DataFrame(
        columns=[
            "代號",
...
def _get_twse_fundamental(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}{month:02}{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.twse.com.tw/exchangeReport/BWIBBU_d?response=csv&date={new_date}&selectType=ALL"
            )
            # 整理資料，變成表格
            df = pd.read_csv(StringIO(r.text.replace("=", "")), header=1)
            # 去除各個欄位名稱後方的多餘空格
            df.columns = [each.strip() for each in df.columns]
            # 更新名稱與代號欄位的資料型態
            df["證券名稱"] = df["證券名稱"].astype(str)
            ...
            print(f"Attempt {_get_twse_fundamental.__name__} failed.")
            time.sleep(3)
    return pd.DataFrame(
        columns=[
            "代號",
...
def _get_twse_margin_trading(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}{month:02}{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.twse.com.tw/rwd/zh/marginTrading/TWT93U?response=csv&date={new_date}&selectType=ALL"
            )
            # 整理資料，變成表格
            ...
            # 去除多餘欄位
            df = df.drop(
                ["現券", "限額", "當日調整", "次一營業日可限額", "備註", "Unnamed: 15"],
                axis=1,
                inplace=False,
            )
            # 取出上市股票（4碼）
            ...
            print(f"Attempt {_get_twse_margin_trading.__name__} failed.")
            time.sleep(3)
    return pd.DataFrame(
        columns=[
            "代號",
...
def _get_twse_institutional(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}{month:02}{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.twse.com.tw/rwd/zh/fund/T86?response=csv&date={new_date}&selectType=ALL"
            )
            # 整理資料，變成表格
            df = pd.read_csv(StringIO(r.text.replace("=", "")), header=1)
            # 去除各個欄位名稱後方的多餘空格
            ...
            print(f"Attempt {_get_twse_institutional.__name__} failed.")
            time.sleep(3)
    return pd.DataFrame(
        columns=[
            "代號",
...
def _get_twse_hold_percentage(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}{month:02}{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.twse.com.tw/rwd/zh/fund/MI_QFIIS?response=csv&date={new_date}&selectType=ALLBUT0999"
            )
            # 整理資料，變成表格
            df = pd.read_csv(StringIO(r.text.replace("=", "")), header=1)
            # 去除各個欄位名稱後方的多餘空格
            ...
            print(f"Attempt {_get_twse_hold_percentage.__name__} failed.")
            time.sleep(3)
