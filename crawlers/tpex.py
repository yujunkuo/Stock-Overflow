...
def get_tpex_final(date):
    ...
    try:
        ...
        df = pd.merge(
            df, tpex_margin_trading_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        df = pd.merge(
            df, tpex_institutional_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        df = pd.merge(
            df, tpex_hold_percentage_df, how="left", on=["代號", "名稱", "股票類型"]
        )
        # 沒有三大法人買賣超的補上0
        df[
            [
...
def _get_tpex_price(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            r = requests.get(
                f"https://www.tpex.org.tw/web/stock/aftertrading/otc_quotes_no1430/stk_wn1430_result.php?l=zh-tw&o=csv&d={new_date}&se=AL&s=0,asc,0"
            )
            # 整理資料，變成表格
            r.encoding = "Big5"
...
def _get_tpex_fundamental(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}/{month:02}/{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.tpex.org.tw/web/stock/aftertrading/peratio_analysis/pera_result.php?l=zh-tw&o=csv&charset=UTF-8&d={new_date}&c=&s=0,asc"
            )
            # 整理資料，變成表格
            r.encoding = "utf8"
            df = pd.read_csv(StringIO(r.text), header=3)
...
def _get_tpex_margin_trading(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}/{month:02}/{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.tpex.org.tw/web/stock/margin_trading/margin_balance/margin_bal_result.php?l=zh-tw&o=csv&charset=UTF-8&d={new_date}&s=0,asc"
            )
            # 整理資料，變成表格
            r.encoding = "utf8"
...
def _get_tpex_institutional(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            new_date = f"{year}/{month:02}/{day:02}"  # 生成符合 url query 的日期字串
            r = requests.get(
                f"https://www.tpex.org.tw/web/stock/3insti/daily_trade/3itrade_hedge_result.php?l=zh-tw&o=csv&se=AL&t=D&d={new_date}&s=0,asc"
            )
            # 整理資料，變成表格
            r.encoding = "Big5"
...
def _get_tpex_hold_percentage(date) -> pd.DataFrame:
    for _ in range(MAX_REQUEST_RETRIES):
        try:
            ...
            r = requests.post(
                "https://mops.twse.com.tw/server-java/t13sa150_otc", data=data
            )
            ...
            hold_percentage_list = [
                [data[x].text, data[x + 1].text, data[x + 6].text]
                for x in range(0, len(data), 11)
            ]
            df = pd.DataFrame(
                hold_percentage_list, columns=["代號", "名稱", "外資持股比率(%)"]
            )
            # 去除換行字元
            df["外資持股比率(%)"] = df["外資持股比率(%)"].apply(
